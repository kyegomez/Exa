...
def cluster_init():
    ...
    logger.info(f"Total available memory across all GPUs: {total_available_memory} GB")
    ...
    if torch.cuda.is_available():
        ...
        logger.info(f"Model memory consumption: {model_memory_consumption}")

        available_memory_after_load = gpu_ops.available_memory_after_model_load(model)
        logger.info(f"Available memory after model load: {available_memory_after_load}")
    else:
