...
class Quantize:
    ...
    def load_model(self):
        ...
        try:
            ...
            self.logger.error(
                f"An error occured while loading the model: {error} please understand the root cause then look at documentation for common errors"
            )
    ...
    def push_to_hub(self, hub):
        ...
        try:
            ...
            if self.model is not None and self.tokenizer is not None:
                ...
                raise ValueError(
                    "Model and tokenizer must be loaded before pushing to the hub"
                )
        except RuntimeError as error:
