...
def initialize_distributed():
    ...
    logger.info(f"Initialized process group with {world_size} processes. Rank {rank} ready to go!")

    # Add checks for initialization errors
    if not dist.is_initialized():
        logger.error("Failed to initialize the distributed process group.")
        raise RuntimeError("Failed to initialize the distributed process group.")
